...
const badge = {
  ...
  active: { cls: 'text-accent2', bg: 'rgba(108,99,255,0.15)', label: '⚡ Active' },
    ...
    <section id="learning" className="py-20 bg-bg">
      <div className="max-w-5xl mx-auto px-6">
        <div className="reveal mb-10">
          <p className="font-mono text-accent text-xs tracking-widest uppercase mb-2">My roadmap</p>
          ...
          {learning.map((item) => {
            ...
            return (
              <div key={item.title}
                ...
                <span className="inline-block font-mono text-xs px-2 py-0.5 rounded mb-3 tracking-wider uppercase"
                      style={{ color: b.cls.replace('text-', ''), background: b.bg,
                               color: item.status === 'done' ? '#00d4aa' : item.status === 'active' ? '#6c63ff' : '#ff6b6b' }}>
                  {b.label}
                </span>
                <div className="font-semibold text-textmain text-sm mb-1.5">{item.title}</div>
